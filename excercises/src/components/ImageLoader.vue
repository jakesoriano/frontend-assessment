...
onMounted(() => {
    if (isObserverSupported && props.lazyLoad) {
        observer.observe(imageHolder.value)
    }
})
...
<template>
    <div :class="`image-loader w-100 ${((lazyLoad || !imageUrl) ? 'not-loaded' : '')} ${containerClass}`" ref="imageHolder">
        <img class="image-loader__img w-100 h-100" :src="(!lazyLoad && imageUrl) ? imageUrl : null" :data-src="imageUrl">
    </div>
</template>
